...
x_data = np.loadtxt(open("x_data%d"%(wtf),"rb"),delimiter=" ",skiprows=0)
y_data = np.loadtxt(open("y_data%d"%(wtf),"rb"),delimiter=" ",skiprows=0)
local = np.loadtxt(open("local%d"%(wtf),"rb"),delimiter=" ",skiprows=0)
...
def bh1(x):
    ...
    return d
x = tf.placeholder(tf.float32, [None,44+wwtf])
