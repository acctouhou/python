...
x_data=np.zeros((9604,135))
y_data=np.zeros((9604,15))
for i in range(9604):
    x_data[i,:]=i
    y_data[i,:]=i
...
def data_train(x_data,y_data,locl,a):
    ...
    to=np.hstack((x_train,y_train,l_train))
    np.random.shuffle(to)
    x_train=to[:,range(135)]
    y_train=to[:,range(135,149)]
    l_train=to[:,[150,151,152]]
...
for j in range(2):
    ...
    writer = tf.python_io.TFRecordWriter("%svali.tfr"%(j))
