...
def cc(inputs,aa,num):
    with tf.variable_scope('resnet%d'%(aa))as scope:
        w1 = tf.Variable(wwtf(shape=[num,num]), name='w1')
        w2 = tf.Variable(wwtf(shape=[num,num]), name='w2')
        b1 = tf.Variable(tf.zeros(shape=[num]))
        b2 = tf.Variable(tf.zeros(shape=[num]))
        ...
        bn2 = tf.nn.elu(tf.layers.batch_normalization(h2, training=train,epsilon=1e-9,momentum=0.99,fused=True)+inputs)
    return bn2
def r1(_x):
...
rn0=cc(bn4,0,1024)
for i in range(10):
    exec("rn%d=cc(rn%d,%d,1024)"%(i+1,i,i+1))

h4 = tf.matmul(rn10,W5) + b5
...
bn11 = tf.layers.batch_normalization(tf.matmul(bn9,W10) + b10+x_center, training=train,epsilon=1e-9,momentum=0.99,fused=True,renorm=True,renorm_clipping=dict)
ans=bn11
